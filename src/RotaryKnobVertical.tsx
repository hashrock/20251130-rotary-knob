import { useRotaryKnobVertical, START_ANGLE, END_ANGLE } from './useRotaryKnobVertical';
...
interface RotaryKnobVerticalProps {
  value?: number;
  min?: number;
  max?: number;
  size?: number;
...
export function RotaryKnobVertical({
  value = 0,
  min = 0,
  max = 100,
  size = 100,
  onChange,
}: RotaryKnobVerticalProps) {
  const { knobRef, valueToAngle, handleMouseDown, handleTouchStart } = useRotaryKnobVertical({
    min,
    max,
    value,
    onChange,
