...
import { useGariNoise } from './useGariNoise';
...
interface UseRotaryKnobGariOptions {
  min: number;
  max: number;
  value: number;
...
export function useRotaryKnobGari({ min, max, value, onChange }: UseRotaryKnobGariOptions) {
  const knobRef = useRef<SVGSVGElement>(null);
  const startAngleRef = useRef(0);
  const startValueRef = useRef(0);
  const { checkAndPlayGari } = useGariNoise();
